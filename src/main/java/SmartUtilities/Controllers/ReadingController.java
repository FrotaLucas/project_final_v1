...
public class ReadingController {
    ...
    public Response addReading(Reading reading) {
        ...
        if (successAdd) {
            ...
            readingData.put("required",
                    List.of("customer", "dateOfReading", "meterId", "substitute", "meterCount", "kindOfMeter"));
    ...
    // 4. Liste von Ablesungs-Objekten: PERGUNTAR PROFESSOR. Acredito que nao tem
    // que mostrar nada do customer pq eh a lista completa de Ablesungen
    ...
    public Response getReadings(@QueryParam("customer") String customerId, @QueryParam("start") String start,
        ...
        if (readings.size() != 0) {
            ...
            readingData.put("items", Map
                    .of("type", "object",
                            "required",
                            List.of("id", "customer", "dateOfReading", "meterId", "substitute", "meterCount",
                                    "kindOfMeter"),
                            "properties", readings));
    ...
    // mudar nome de _readingService.updateNewReading() para
    // _readingService.updateReading();
    @PUT
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
